...
namespace CommonUserControls
{
    public partial class UserControlCustomers : UserControlBase, ICustomers, IValidate,IDataSource
    {
       ...
       private int _Id;
        ...
        public object DataGridSource { get => DataGridData.DataSource; set => DataGridData.DataSource = value; }
        ...
        private void DataGridData_DoubleClick(object sender, EventArgs e)
        {
            if (this.DataGridData.Rows.Count > 0)
            {
                int id = (int)this.DataGridData.SelectedRows[0].Cells[nameof(Customers.Id)].Value;
